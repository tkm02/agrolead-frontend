...
// Suppression de l'import inutilisé de framer-motion
...
const ProductPage = () => {
    ...
    const handleCategoryChange = (category) => {
      // Si category est "Tous", affichez tous les produits
      if (category === "Tous") {
        setFilteredProducts(validProducts);
      } else {
        // Sinon, filtrez par catégorie
        const filtre = validProducts.filter(product => product.type === category);
    ...
    useEffect(() => {
      ...
      // Cette partie n'est pas nécessaire car elle réinitialise filteredProducts à chaque démontage
      // return () => {
      //     setFilteredProducts([]);
      // };
